...
def _run_round_diagnostics(
    ...
    if lower_clip_percentile is None and lower_clip_value is None:
        effective_lower_clip_value = float(np.percentile(train_lnls, 5.0))
...
def offline_surrogate_diagnostics(
    ...
    Args:
        evaluator: Callable accepting a parameter vector and returning LnL.
        bounds: Parameter bounds array with shape (2, D).
        n_train: Number of training evaluations to draw (used if round_totals is None).
        n_test: Number of evaluation points for diagnostics.
        round_totals: Optional strictly increasing sequence of cumulative training counts.
        seed: Random seed for reproducibility.
        scaler_kwargs: Optional overrides passed to AdaptiveRobustScaler.
        outdir: Optional directory for diagnostic plots.
        truths: Optional mapping of parameter names to true values for MCMC corner plots.
        mcmc_kwargs: Optional overrides for the MCMC sampler.

    Returns:
        OfflineDiagnosticsResult with metrics and plotting artefacts.
    """
    ...
    for round_idx, target_total in enumerate(round_totals):
        ...
        round_dir = base_dir / f"round_{round_idx:02d}_N{train_points.shape[0]}" if base_dir is not None else None
