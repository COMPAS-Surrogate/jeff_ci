...
def test_lnl(test_compas_h5, observation_file, outdir):
    ...
    # ensure that lnl is finite
    assert np.isfinite(lnl), f"Log likelihood should be finite, got {lnl}"
...
def test_lnl_1d(test_compas_h5, observation_file, outdir):
    ...
    """
    np.random.seed(42)
    outdir = os.path.join(outdir, "lnl_1d_test")
    os.makedirs(outdir, exist_ok=True)
    ...
    assert result.exit_code == 0, f"Click command failed: {result.output}"
